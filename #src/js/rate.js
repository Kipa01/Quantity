let ratingItemList = document.querySelectorAll('.rating-product__item'); // создаем массив из элементов рэйтинга(звездочек)
...
ratinItemsArray.forEach(item => { // идем по массиву
   item.addEventListener('click', function (e) { // при клике вызывается ф-ия
      ...
      let valRate = item.closest('.rating-product__body'); // записываем в новую переменную блок, внутри которого наши звездочки( блок, у которого атрибут отвечает за кол-во активных звездочек)
